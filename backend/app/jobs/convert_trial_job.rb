class ConvertTrialJob < ApplicationJob
  ...
  def perform(subscription_id)
    ...
    begin
      if subscription.customer.standing_order_enabled?
        # If customer has standing order, create billing attempt atomically
        with_transaction do
      ...
      # If conversion fails, suspend the subscription (outside transaction)
      subscription = Subscription.find_by(id: subscription_id)
      if subscription
        subscription.update!(
          status: 'suspended',
        ...
        SubscriptionMailer.trial_conversion_failed(subscription).deliver_later
      end
      raise
    end
  end
  ...
  def initiate_stk_push_for_trial(subscription)
    ...
    # Create billing attempt and initiate STK Push atomically
    with_transaction do
      ...
      response = SafaricomApi.client.mpesa.stk_push.initiate(
        ...
        transaction_desc: "Trial conversion: #{subscription.name}",
      ...
      send_sms(customer.phone_number,
               "Your trial period has ended. Please complete payment of KES #{amount} to continue your #{subscription.name} subscription. Check your phone for M-Pesa prompt.")
    end
  rescue StandardError => e
    Rails.logger.error("Failed to initiate STK Push for trial conversion #{subscription.id}: #{e.message}")
    # Re-raise to be caught by main rescue block
    raise e
  end
